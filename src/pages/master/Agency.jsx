...
const Agency = () => {
  ...
  const editAgency = async (id) => {
    try {
      const payload = encryptPayload({ agencyId: id });
      const res = await editAgencySerice(payload);
      // console.log(res);
      if (res?.status === 200 && res?.data.outcome) {
        setFormData(res?.data.data);
        setFormData((prev) => ({ ...prev, dob: formatDateMMDDYY(prev.dob) }));
        setRows(res?.data.data.agencyBankDetailsDtoList);
        setExpanded("panel1");
      }
    } catch (error) {
      throw error;
  ...
  const toggleStatus = async () => {
    try {
      const payload = encryptPayload({ agencyId: openMilestoneId });
      const res = await toggleAgencyStatus(payload);
      //   console.log(res);
      if (res?.status === 200 && res?.data.outcome) {
        setOpenModal(false);
            ...
            >
              ...
              <div className="col-span-12">
                <div className="flex justify-center gap-2 text-[13px] bg-[#42001d0f] border-t border-[#ebbea6] px-4 py-3 rounded-b-md">
                  <ResetBackBtn />
                  <SubmitBtn type={"submit"} btnText={agencyId} />
                </div>
              </div>
            </form>
          </div>
